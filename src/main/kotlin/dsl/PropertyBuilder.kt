...
class PropertyBuilder(private val cozy: Cozy<PropertyBuilder> = Cozy()) :
    ...
    fun mutable() { source.mutable(true) }
    fun immutable() { source.mutable(false) }
    fun transient() { source.transient() }
    fun volatile() { source.volatile() }

    fun initializer(assembler: Assembler<CodeBuilder>) { source.initializer(CodeBuilder().buildWith(assembler)) }
